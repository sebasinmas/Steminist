...
const Header: React.FC<HeaderProps> = ({ notificationCount, theme, toggleTheme }) => {
    ...
    const navLinks: NavLinkInfo[] = [
        ...
        { path: '/dashboard', label: 'Panel de Control', roles: ['mentee', 'mentor'] },
            ...
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex items-center justify-between h-20">
                    <div className="flex items-center">
                        <NavLink to="/" className="flex-shrink-0 font-bold text-2xl cursor-pointer text-primary">
                            MentorHer
                        </NavLink>
                    </div>
                    <nav className="hidden md:flex items-center space-x-8">
                        {role && navLinks.filter(link => link.roles.includes(role)).map(link => (
                            <NavLink
                                key={link.path}
                                to={link.path}
                                className={({ isActive }) => `text-lg font-medium transition-colors ${isActive ? activeLinkClass : inactiveLinkClass}`}
                            >
                                {link.label}
                            </NavLink>
                        ))}
                    ...
                    <div className="flex items-center space-x-4">
                        ...
                        {role !== 'admin' && (
                            <NavLink to="/notifications" className="relative p-2 rounded-full hover:bg-accent transition-colors">
                                <BellIcon />
                                {notificationCount > 0 && (
                            ...
                            <div className="relative hidden md:block">
                                ...
                                {isDropdownOpen && (
                                    <div className="absolute right-0 mt-2 w-48 bg-card border border-border rounded-md shadow-lg py-1">
                                        {role !== 'admin' && <button onClick={() => { navigate('/profile'); setIsDropdownOpen(false); }} className="block w-full text-left px-4 py-2 text-sm text-foreground hover:bg-accent">Mi Perfil</button>}
                                        <div className="border-t border-border my-1"></div>
                                        <button onClick={handleLogout} className="block w-full text-left px-4 py-2 text-sm text-red-500 hover:bg-accent">Cerrar Sesión</button>
                                    </div>
                                )}
                            </div>
                        )}
                    ...
                    <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
                         {role && navLinks.filter(link => link.roles.includes(role)).map(link => (
                            <NavLink
                                ...
                                className={({ isActive }) => `block w-full text-left px-3 py-2 rounded-md text-base font-medium ${isActive ? 'bg-accent text-primary' : 'text-foreground/80 hover:bg-accent'}`}
                            >
                                {link.label}
                            </NavLink>
                        ))}
                        ...
                        <div className="border-t border-border pt-4 mt-4">
                             <button onClick={() => { handleLogout(); setIsMenuOpen(false); }} className="block w-full text-left px-3 py-2 rounded-md text-base font-medium text-red-500 hover:bg-accent">Cerrar Sesión</button>
                        </div>
                    </div>
                </div>
            )}
