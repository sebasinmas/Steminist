                    ...
                    <div className="mt-4">
                        <div className="flex items-center gap-2">
                            <input
                                ...
                                className="grow p-2 border border-border rounded-md bg-input text-foreground"
                            />
                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    {isMentor && (
                        <div className="bg-card p-6 rounded-lg border border-border">
                            ...
                            <div className="space-y-3">{((profileData as Mentor).links || []).length > 0 ? (((profileData as Mentor).links || []).map((link, index) => (<div key={index} className="flex items-center justify-between group bg-secondary/50 p-3 rounded-md"><a href={link.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-primary hover:underline"><LinkIcon className="w-5 h-5 shrink-0" /><span className="font-semibold truncate">{link.title}</span></a>{isEditing && (<button onClick={() => handleRemoveLink(index)} className="bg-red-500 text-white rounded-full h-6 w-6 flex items-center justify-center text-xs opacity-0 group-hover:opacity-100 transition-opacity shrink-0 ml-4" aria-label={`Eliminar enlace ${link.title}`}><XIcon className="h-4 w-4" /></button>)}</div>))) : (!isEditing && <p className="text-muted-foreground">No se han añadido enlaces.</p>)}</div>
