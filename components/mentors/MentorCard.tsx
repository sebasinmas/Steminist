...
import type { Mentor, Page } from '../../types';
import type { MatchDetails, MatchStatus } from '../../pages/MentorSearchPage';
...
// A self-contained component for the affinity score visualization
const CircularProgress: React.FC<{ percentage: number }> = ({ percentage }) => {
    ...
    React.useEffect(() => {
        ...
        // Set a timeout to allow the initial render to happen before the animation starts
...
const MentorCard: React.FC<MentorCardProps> = ({ mentor, navigateTo, matchDetails }) => {
    const [isBreakdownVisible, setIsBreakdownVisible] = useState(false);

    return (
        <Card 
            className="flex flex-col h-full transition-shadow duration-300 hover:shadow-2xl"
        >
            <div 
                className="flex-grow cursor-pointer"
                onClick={() => navigateTo('profile', mentor)}
            >
            ...
            <div className="mt-auto pt-4 border-t border-border">
                <div className="flex justify-between items-center">
                    ...
                    <Button size="sm" onClick={(e) => { e.stopPropagation(); navigateTo('profile', mentor); }}>Ver Perfil</Button>
                </div>
